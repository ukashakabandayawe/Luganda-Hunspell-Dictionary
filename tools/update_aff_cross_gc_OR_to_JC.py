...
# Cross product generator: gc x OR => JC
# Description:
# - Left block `gc`: gc
# - Right block `OR`: Special reflexive object markers
# - Output flag `JC`: Cross-product prefixes for gc x OR
...
INSERT_BEFORE_FLAG = "HB".strip() or None

rule_left_raw = """
PFX gc Y 19
PFX gc j bwenzi jj # It creates wenzija from stem "jja"
PFX gc 0 bwen [^jbnmlhprxq] # Person marker
PFX gc 0 bwem [b] # Person marker
PFX gc l bwend [l] # Person marker
PFX gc 0 bwetu . # Person marker
PFX gc 0 bwemu . # Person marker
PFX gc 0 bweba . # Person marker
PFX gc 0 bwegu .
PFX gc 0 bwegi .
PFX gc 0 bwezi .
PFX gc 0 bweki .
