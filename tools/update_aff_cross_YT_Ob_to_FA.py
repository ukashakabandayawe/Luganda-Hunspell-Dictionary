...
# Cross product generator: YT x Ob => FA
# Description:
# - Left block `YT`: adverb " yet" subject markers  hardcoded from Luganda.aff
# - Right block `Ob`: basic object markers (capital Ob) hardcoded from Luganda.aff
# - Output flag `FA`: pre-generated cross-product prefixes for YT x Ob
#
# The script parses the hardcoded rule blocks, multiplies them respecting strip/add compatibility
# and Hunspell-style conditions, then removes existing FA lines in Luganda.aff and appends the new block.
...
def parse_rules(raw_text):
    ...
    for line in raw_text.strip().split('\n'):
        ...
        if len(parts) < 4:
            continue
        # Skip header lines like: PFX FLAG Y N
        if parts[2] == 'Y':
            continue
...
def generate_block():
    ...
    for left in lefts:
        for right in rights:
            ...
            # 2) condition compatibility: apply left cond to right add
            if left['cond'] != '.':
